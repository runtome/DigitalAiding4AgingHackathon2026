...
from config import (
    DOMINANCE_MARGIN,
    STATS_CI_LEVEL, STATS_NORMALITY_ALPHA, STATS_SIGNIFICANCE_ALPHA,
    COHEN_D_NEGLIGIBLE, COHEN_D_SMALL, COHEN_D_MEDIUM,
)
...
def make_speed_normal_dist_chart(events: list) -> go.Figure:
    ...
    def _desc(arr):
        ...
        ci   = sp_stats.t.interval(STATS_CI_LEVEL, df=n - 1, loc=mean, scale=se)
    ...
    row_heights = [0.22, 0.24, 0.54] if bilateral else [0.45, 0.55]
    specs = (
    ...
    # Subplot title font size (annotations generated by make_subplots)
    ...
    fig.update_xaxes(title_text="Reaction Time (ms)", title_font_size=14,
                     tickfont_size=13, row=1, col=1)
    fig.update_yaxes(title_text="Probability Density", title_font_size=14,
                     tickfont_size=13, row=1, col=1)
    ...
    _STAT_ROWS = [
        ...
        ("Minimum",        "mn",    ".0f", "ms"),
        ("Maximum",        "mx",    ".0f", "ms"),
        (f"{int(STATS_CI_LEVEL*100)}% CI Lower", "ci_lo", ".1f", "ms"),
        (f"{int(STATS_CI_LEVEL*100)}% CI Upper", "ci_hi", ".1f", "ms"),
        ("CV (%)",         "cv",    ".1f", "%"),
    ]
    ...
    fig.add_trace(go.Table(
        header=dict(
            ...
            fill_color=_HDR, font=dict(color="white", size=14), align="left",
        ),
        cells=dict(
    ...
    if bilateral:
        ...
        def _shapiro(arr):
            ...
            return float(st), float(pv), bool(pv > STATS_NORMALITY_ALPHA)
        ...
        sig_str = (
            f"✓ Significant (p < {STATS_SIGNIFICANCE_ALPHA})"
            if (not np.isnan(pv_t) and pv_t < STATS_SIGNIFICANCE_ALPHA)
            else f"✗ Not significant (p ≥ {STATS_SIGNIFICANCE_ALPHA})"
        )
        ...
        eff_cls = (
            "Negligible" if d < COHEN_D_NEGLIGIBLE else
            "Small"      if d < COHEN_D_SMALL      else
            "Medium"     if d < COHEN_D_MEDIUM      else
            "Large"
        )
        ...
        _ROWS = [
            ...
            (False, "Classification",  eff_cls  + f"  ({COHEN_D_NEGLIGIBLE} negligible | {COHEN_D_SMALL} small | {COHEN_D_MEDIUM} medium)"),
...
def make_quality_chart(events: list, analysis=None) -> go.Figure:
    ...
    if analysis:
        for prefix, color, name in [("right", "#2ecc71", "Right"), ("left", "#3498db", "Left")]:
            ...
            if eff:
                eff_pct = [v * 100 for v in eff]
                fig.add_trace(go.Scatter(y=eff_pct, name=f"{name} Efficiency",
